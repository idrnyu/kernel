...
static int rockchip_mpp_h265e_set_gop_parameter(struct rockchip_mpp_dev *mpp,
						int index);
static int rockchip_mpp_h265e_register_frame_buffer(struct rockchip_mpp_dev *mpp,
						    int index);
...
static int rockchip_mpp_h265e_load_firmware(struct rockchip_mpp_dev *mpp)
{
	...
	enc->firmware.hdl = rockchip_mpp_h265e_global_dma_alloc(mpp,
								firmware->size,
								MPP_ALIGN_SIZE,
...
static struct mpp_session *rockchip_mpp_h265e_open(struct rockchip_mpp_dev *mpp)
{
	...
	if (!atomic_read(&enc->load_firmware)) {
		...
		enc->temp.hdl = rockchip_mpp_h265e_global_dma_alloc(mpp,
								    enc->temp.size,
								    MPP_ALIGN_SIZE,
								    &enc->temp.dma_addr);
		if (enc->temp.hdl < 0) {
